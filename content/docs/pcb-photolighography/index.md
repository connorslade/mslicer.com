...
The song is Destiny by Thaiboy Digital btw.
...
I've been using the dry film, which works great as long as you get it applied correctly.
...
Then cut a square of the photoresist a bit larger than your copper clad, peel the protective film off one side (with two pieces of tape), and stick it down using a credit card to get out any air bubbles.
...
In order to transfer your pattern from a Gerber file to the photoresist, it needs to be exposed with ~400 nm UV light, which is the same as what UV curing resin uses!
...
Once you have your PCB design (I use KiCad btw) export at least the front copper and edge cut layers to Gerber files (also back copper if you're doing double-sided).
...
In mslicer, open the PCB tool from the '<i class="ph ph-hammer"></i> Tools' dropdown in the top bar.
Select 'Load Gerbers' and select the copper and edge cuts layers.

Each layer has two checkboxes: layer visibility (<i class="ph ph-eye"></i>) and whether it's considered part of the bounding box (<i class="ph ph-bounding-box"></i>).
...
I've gotten good results with 4096× antialiasing and a linear exposure curve.
I think this is because it underexposes the edges of traces, somewhat compensating for unwanted light spread.
...
Make sure that the front layer is exposed without a flip and only the bottom layer is flipped!

![](sliced-pcb.png)

### Locating the Board

Here is what I came up with for locating the board over your printer LCD.
It's a milled sheet of 2 mm thick acrylic with five interior holes.
...
If you encounter any bugs in mslicer please [open an issue](https://github.com/connorslade/mslicer/issues/new/choose) on GitHub, if you have any questions, feel free to [reach out](https://connorslade.com/#contact:~:text=Contact).
